...
def trades_loss(
    ...
    if r < cutmix_prob:
        ...
        x_natural, targets_a, targets_b, lam = mixup_data(x_natural, y,
                                                       1, True)
        x_natural, targets_a, targets_b = map(Variable, (x_natural,
                                                      targets_a, targets_b))
        # compute output
        output = model(x_natural)

        loss_natural = mixup_criterion(natural_criterion, output, targets_a, targets_b, lam)
    else:
        # compute output
        output = model(x_natural)
