...
from src.utils.weight_noise import udr_pgd,udr_pgd_1
...
def CAP_loss(
    ...
    # else:
    #     x_adv = torch.clamp(x_adv, clip_min, clip_max)
    # lamda = torch.tensor(1, requires_grad=False).cuda()
    x_adv, delta = udr_pgd_1(x_natural,x_aug, y ,model,perturb_steps, 0.01, tau,lamda=lamda,device=device,epsilon=epsilon,args=args)
    model.train()
    ...
    return loss, loss_natural.mean(), loss_robust.mean(),delta


def train(model, dataloader, optimizer, args, epoch, device, logger, cap_adversary, AttackPolicy,lamda,tau):
    ...
    for i, data in enumerate(dataloader):
        if args.aug=='sgf':
            ...
            x_aug = data[0].to(device)
            label = data[1].to(device)

        nat_output = model(x_natural)
        # calculate robust loss
        loss, nat_loss, rob_loss,delta = CAP_loss(
            model=model,
            x_natural=x_natural,
            x_aug=x_aug,
            y=label,
    ...
    return model, losses.avg, nat_losses.avg, rob_losses.avg, top1.avg,delta
